...
app.post("/jokes", (req, res) => {
  ...
  if (!jokeText || !jokeType) {
    return res.status(400).send("Joke text and type are required")
  }
  const newJoke = {
    id: jokes.length + 1,
    jokeText,
...
app.put("/jokes/:id", (req, res) => {
  const { id } = req.params
  const { jokeText, jokeType } = req.body
  const jokeIndex = jokes.findIndex(j => j.id === parseInt(id))
  if (jokeIndex === -1) {
  ...
  jokes[jokeIndex] = { id: parseInt(id), jokeText, jokeType }
...
app.patch("/jokes/:id", (req, res) => {
  ...
  if (!jokeText && !jokeType) {
    return res.status(400).send("At least one of joke text or type is required")
  }
  jokes[jokeIndex] = { 
    id: parseInt(id), 
    jokeText: jokeText || jokes[jokeIndex].jokeText, 
    jokeType: jokeType || jokes[jokeIndex].jokeType 
  }
  res.json(jokes[jokeIndex])
})
...
app.delete("/all", (req, res) => {
  const { key } = req.query
  // masterKey on .env file
  if (key !== process.env.masterKey) {
    return res.status(403).send("Forbidden: Invalid master key")
  }
  jokes = []
  res.status(204).send()
})
